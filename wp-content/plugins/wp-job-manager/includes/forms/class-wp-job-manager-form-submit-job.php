...
class WP_Job_Manager_Form_Submit_Job extends WP_Job_Manager_Form
{
     ...
     */
    public function init_fields($type = 'procurement')
    {
        if ($this->fields) {
            return;
        }
        ...
        //$job_type_id = $this->get_job_type_id_by_slug($type);
        ...
        $this->fields = apply_filters('submit_job_form_fields', array(
            'job' => array(
                 ...
                 * 
                 */
                'job_upper_price' => array(
                    'label' => __( ( $type == 'procurement' ? 'Upper price' : 'Inital price'), 'wp-job-manager'),
                    //'description' => __('Upper price', 'wp-job-manager'),
                    'type' => 'text',
                    'required' => false,
                    'placeholder' => __('e.g. "100 EUR"', 'wp-job-manager'),
                    'priority' => 2
                ),
                'job_delivery_time' => array(
                    'label' => __('Delivery time', 'wp-job-manager'),
                    //'description' => __('Upper price', 'wp-job-manager'),
                    'type' => 'date',
                    'required' => true,
                    'placeholder' => '',
                ...
                'job_start_time' => array(
                    'label' => __('Start time', 'wp-job-manager'),
                    //'description' => __('Upper price', 'wp-job-manager'),
                    'type' => 'date',
                    'required' => true,
                    'placeholder' => __('e.g. "25.02.2025"', 'wp-job-manager'),
                    'priority' => 4
                ),
                'job_deadline' => array(
                    'label' => __('Deadline', 'wp-job-manager'),
                    //'description' => __('Upper price', 'wp-job-manager'),
                    'type' => 'date',
                    'required' => true,
                    'placeholder' => __('e.g. "25.02.2025"', 'wp-job-manager'),
                ...
                'company_images' => array(
                    'label' => __('Images', 'wp-job-manager'),
                    'type' => 'file',
                    'required' => false,
                    'placeholder' => '',
                    'priority' => 7,
                ...
                'job_payment' => array(
                    'label' => __('Payment', 'wp-job-manager'),
                    'type' => 'wp-editor',
                    'required' => true,
                    'placeholder' => '',
                    'priority' => 11
        ...
        if($type === 'procurement') {
            unset($this->fields['job']['job_delivery_time']);
            unset($this->fields['job']['job_start_time']);
        }
    ...
    public function buy($attr = array())
    {
        $type = 'procurement';
        if(is_array($attr) && key_exists('job_types', $attr)) {
            $type = $attr['job_types'];
        }
        if($this->has_rights_to_submit($type)) {
            $this->step ++;
            $this->submit($attr);
        } else {
            echo do_shortcode('[product sku="'.$type.'"]');
        }
    }
    
    function has_rights_to_submit($type)
    {
        $ordersCount = $this->get_job_offer_product_count($type);
        if($ordersCount == 0) {
            return false;
        }
        
        $jobsCount = $this->get_customer_jobs_count($type);
        if($jobsCount > $ordersCount) {
            return false;
        }
    ...
    function get_customer_jobs_count($type)
    {
        global $wpdb;
    ...
    function get_job_type_id_by_slug($slug)
    {
        global $wpdb;
        
        $sql = "SELECT tt.term_taxonomy_id AS id "
    ...
    {
        $types = array('procurement', 'offer');
        $taxonomy = 'job_listing_type';
        
        foreach($types AS $type) {
            
            $term = term_exists( $type, $taxonomy );
            if($term) {
                continue;
            }
            
            wp_insert_term($type, $taxonomy, array(
                'slug' => $type,
                'description' => $type
            ));
    ...
    function get_job_offer_product_count($type)
    {
        $qty = 0;
    ...
    function create_new_job_offer_product($type)
    {
        $title = $type === 'procurement' ? __('Hange', 'wp-job-manager') : __('Pakkumine', 'wp-job-manager');
        ...
        update_post_meta( $post_id, '_sku', $type );
     ...
     */
    public function submit_handler($attr = array())
    {
        $type = 'procurement';
        if(is_array($attr) && key_exists('job_types', $attr)) {
            $type = $attr['job_types'];
