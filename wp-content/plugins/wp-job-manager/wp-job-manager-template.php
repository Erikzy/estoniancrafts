...
function get_the_job_upper_price($post = null) {
    $post = get_post( $post );
    if ( $post->post_type !== 'job_listing' ) {
        return;
    }
    
    return apply_filters( 'the_job_upper_price', $post->_job_upper_price, $post );
}

function get_the_job_current_price($post = null) {
    global $wpdb;
        
    $post = get_post($post);

    $table_name = $wpdb->prefix . 'job_offers';
    $ids = get_post_meta($post->ID, 'job_offers', true);
    if(!is_array($ids)) {
        return get_the_job_upper_price($post);
    }
    ...
    $idsStr = implode(',', $ids);
    if($postType == 'procurement') {
        $sql = "SELECT price FROM $table_name WHERE id in ($idsStr) ORDER BY price ASC LIMIT 1";
    } else {
        $sql = "SELECT price FROM $table_name WHERE id in ($idsStr) ORDER BY price DESC LIMIT 1";
    }
    $results = $wpdb->get_results( $sql, OBJECT );
    
    if($results) {
        return $results[0]->price . ' EUR';
    }
    
    return get_the_job_upper_price($post);
}

function get_the_job_current_bids($post = null) {
    global $wpdb;
    ...
    $count = $wpdb->get_row( $countSql, OBJECT );
...
function get_the_job_deadline($post = null) {
    ...
    return apply_filters( 'the_job_deadline', $post->_job_deadline, $post );
}

function update_the_job_deadline($date, $post = null) {
    $post = get_post( $post );
    if ( $post->post_type !== 'job_listing' ) {
        return;
    }
    
    update_post_meta( $post->ID, '_job_deadline', $date );
}
...
function get_the_job_image($post = null, $field = 'image', $size = 'thumbnail') {
    $post = get_post( $post );
    
    $images = array();
    if(! empty( $post->{'_' . $field} )) {
        $_image = $post->{'_' . $field};
        if(is_array($_image)) {
            foreach($_image AS $img) {
                $images[] = apply_filters( 'the_' . $field, $img, $post );
            }
        } else {
            $images[] = apply_filters( 'the_' . $field, $_image, $post );
        }
    }
    
    foreach($images AS $image) {
        if ( ! empty( $image ) && ( strstr( $image, 'http' ) || file_exists( $image ) ) ) {
            echo "<a href='" . esc_attr( $image ) . "' alt='image'>";
            if ( $size !== 'full' ) {
                $image = job_manager_get_resized_image( $image, $size );
            }
            echo '<img class="iamge" src="' . esc_attr( $image ) . '" alt="' . esc_attr( 'image' ) . '" />';
            echo "</a>";
        }
    }
}

function get_the_job_files($post = null, $field = 'job_file') {
    $post = get_post( $post );
    ...
    if(! empty( $post->{'_' . $field} )) {
        ...
        if(is_array($_file)) {
            foreach($_file AS $file) {
                $files[] = apply_filters( 'the_' . $field, $file, $post );
            }
        } else {
            $files[] = apply_filters( 'the_' . $field, $_file, $post );
        }
    }
    
    foreach($files AS $file) {
        if ( ! empty( $file ) && ( strstr( $file, 'http' ) || file_exists( $file ) ) ) {
            echo '<a class="file" href="' . esc_attr( $file ) . '" alt="' . esc_attr( $file ) . '">' . esc_attr( basename ($file) ) . '</a>';
        }
    }
}
...
function get_the_job_author_info($post = null) {
    $post = get_post( $post );
    if ( $post->post_type !== 'job_listing' ) {
        return;
    }
    ...
    if($user->data->user_url) {
        $profileLink = $user->data->user_url;
    } elseif($user->data->user_login) {
        $profileLink = '/user/' . $user->data->user_login;
    }
...
function get_the_job_payment($post = null) {
    $post = get_post( $post );
    if ( $post->post_type !== 'job_listing' ) {
        return;
    }
    
    return apply_filters( 'the_job_payment', $post->_job_payment, $post );
}

function get_the_job_terms($post = null) {
    $post = get_post( $post );
    if ( $post->post_type !== 'job_listing' ) {
        return;
    }
    
    return apply_filters( 'the_job_terms', $post->_job_terms, $post );
}
...
function add_cron_recurrence_interval_every_minute( $schedules ) {
...
add_filter( 'cron_schedules', 'add_cron_recurrence_interval_every_minute' );

if ( ! wp_next_scheduled( 'every_minute_action_hook' ) ) {
    wp_schedule_event( time(), 'every_minute', 'every_minute_action_hook' );
}

add_action('every_minute_action_hook', 'send_inbox_mail_when_job_deadline');
function send_inbox_mail_when_job_deadline() {
    $jobs = get_customer_expired_jobs();
    $status = 'expired';
    
    foreach($jobs AS $job) {
        
        $userId = get_best_bitter_row_id($job);
        
        if($userId) {
            send_job_expired_message_to_new_member($job->post_author, $userId);
            send_job_expired_message_to_new_member($userId, $job->post_author);
        } else {
            send_job_expired_message_to_new_member($job->post_author, $job->post_author);
        }
...
function get_customer_expired_jobs()
{
    global $wpdb;
    ...
    $sql = "SELECT p.* "
            ...
            . "AND tt.taxonomy =  'job_listing_type' "
    ...
    foreach($results AS $post) {
        $deadline = strtotime( get_the_job_deadline($post) );
        if($now > $deadline) {
...
function get_best_bitter_row_id($post = null) {
    global $wpdb;
    ...
    $idsStr = implode(',', $ids);
    if($postType == 'procurement') {
        $sql = "SELECT * FROM $table_name WHERE id in ($idsStr) ORDER BY price ASC LIMIT 1";
    } else {
        $sql = "SELECT * FROM $table_name WHERE id in ($idsStr) ORDER BY price DESC LIMIT 1";
    }
    $results = $wpdb->get_row( $sql, OBJECT );
    
    if(is_object($results) && $results->user_id) {
...
function send_job_expired_message_to_new_member( $sender_id, $user_id ) {
    $args = array(
        'sender_id' => $sender_id,
        'thread_id' => false,
        'recipients' => array( $user_id ),
        'subject' => __('job_expired_message_subject', 'wp-job-manager'),
        'content' => __('job_expired_message_content', 'wp-job-manager'),
