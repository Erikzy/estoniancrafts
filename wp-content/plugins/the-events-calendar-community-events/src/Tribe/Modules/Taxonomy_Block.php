...
class Tribe__Events__Community__Modules__Taxonomy_Block {
	...
	public static function instance() {
		if ( empty( self::$instance ) ) {
			...
			$allow_anonymous_submissions = tribe( 'community.main' )->allowAnonymousSubmissions;
	...
	public function the_category_checklist( $event = null ) {
		...
		?>
		<div
			id="event-categories" class="tribe-hide"
			...
			<div class="tribe-event-categories">
				 <ul
					 role="group"
					 class="tribe-categories-with-children"
					 aria-label="<?php esc_html_e( 'event categories', 'tribe-events-community' );?>"
				 >
					<?php
					$args = array(
						'checked_ontop' => false,
		...
		if ( $show_all_link ) {
			echo '<div class="tribe-hide"><a id="show_hidden_categories" class="show-hidden-categories" href="">' . sprintf( __( 'Show all categories ( %d )',
