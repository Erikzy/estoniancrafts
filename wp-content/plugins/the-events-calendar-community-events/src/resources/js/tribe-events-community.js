...
(function( window, $ ) {
	...
	tribe_community_events.init_categories = function() {
		...
		var $parents = this.$event_categories.find( '> div > ul > li' );
		...
		if ( ! $children.length ) {
			...
			$parents.each( function() {
				...
				if ( $ul.length ) {
					...
					$label.after( '<span class="tribe-toggle"><span>' + count + ' ' + more_text + '</span></span>' );
				}
			});
		}
	 ...
	 *  320px or less in order to invoke the native datepicker
	 *
	...
	$( document ).ready( function() {
		$mobile_datepicker = $('#EventStartDate, #EventEndDate');
		$mobile_timepicker = $('#EventStartTime, #EventEndTime');
	});
	...
	$window.on( {
		'resize.tribe': function () {
			...
			if ( viewport_width <= 568 ) {
				$mobile_datepicker.prop( 'type', 'date' );
				$mobile_timepicker.timepicker({
					disableTouchKeyboard: true
				})
			} else {
				$mobile_datepicker.prop( 'type', 'text' );
				$mobile_timepicker.timepicker({
